...
#undef STUB_LEVEL
#define STUB_LEVEL(_0, _1, _2, _3, _4, _5, _6, _7, _8)
#define DEFINE_LEVEL(_0, _level, _course, _name, _4, _5, _6, _7, _8, _9, _10) { (s32) _level, (s32) _course, (void *) level_##_name##_entry },
...
static const u8 sCastle[]       = { 12, 10, 28, 29, 21, 14, 255 };
static const u8 sBowser1[]      = { 11, 24, 32, 28, 14, 27, 158, 1, 255 };
...
static void *_GetScript(s32 aLevel) {
    for (const auto &lcs : sLevelCourseScript) {
        if (lcs.mLevel == aLevel) {
            return lcs.mScript;
        }
...
static void _SetLevelName(u8 *aBuffer, s32 aLevel) {
    ...
    const u8 *courseName = ((const u8 **) seg2_course_name_table)[_Course - COURSE_BOB] + 3;
...
static void _PrefixNumber(u8 *aBuffer, s32 aNumber) {
...
static s32 sLevelCount[2]    = { 0, 0 };
static s32 *sLevelList[2][2] = { { NULL, NULL }, { NULL, NULL } };
static s32 *sLevelCourses    = NULL;
...
static void DynOS_Level_Init() {
    ...
    if (!sInited) {
        ...
        // Lists allocation
        sLevelList[0][0] = New<s32>(sLevelCount[0]);
        sLevelList[0][1] = New<s32>(sLevelCount[0]);
        sLevelList[1][0] = New<s32>(sLevelCount[1]);
        sLevelList[1][1] = New<s32>(sLevelCount[1]);
        sLevelCourses    = New<s32>(LEVEL_COUNT);
        sLevelScripts    = New<void *>(LEVEL_COUNT);

        // Level list
        for (s32 i = 0; i != sLevelCount[0]; ++i) {
            sLevelList[0][0][i] = sLevelCourseScript[i].mLevel;
        }

        // Level list ordered by course id
        for (s32 i = 0, k = 0; i < COURSE_END; ++i) {
            for (s32 j = 0; j < sLevelCount[0]; ++j) {
        ...
        for (s32 i = 0, k = 0; i != sLevelCount[0]; ++i) {
        ...
        for (s32 i = COURSE_BOB, k = 0; i < COURSE_CAKE_END; ++i) {
            for (s32 j = 0; j < sLevelCount[0]; ++j) {
                if (sLevelCourseScript[j].mCourse == i) {
